...
/* Calculate number of iterations for a given i index (real axis) and loop over
   all j values (imaginary axis). The nIter array is populated with the results.
 */
...
int main(int argc, char *argv[]) {
  ...
  /* Initialise to nIter zero as we will use a reduce to collate results into
   * this array*/
  for (i = 0; i < N_RE + 1; i++) {
    for (j = 0; j < N_IM + 1; j++) {
      nIter[i][j] = 0;
  ...
  /* Record end time. The barrier synchronises the process so they all measure
   * the same time */
  if (myRank == 0) {
